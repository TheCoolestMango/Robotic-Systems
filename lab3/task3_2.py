...
# pendulum parameters
theta_des = pi/2 # start
dtheta_des = 0
m = 0.1      # mass
l = 0.1      # length
g = 9.81     # grav
b = 0.004    # damping coefficient
...
pendulum.state = array([theta_des, dtheta_des])
pendulum.control = 0 
  ...
  # Control process 
    ...
    while True:
        ...
        if (time - last_execution) >= sampling_time:
            ...
            #PID controller
            pid = odeint(pid_model, array([theta_des,theta]), t, args=(Ki,))
            #print(pid.shape)
            control = Kp*(theta_des - theta) + Kd*(dtheta_des - dtheta) + pid[-1][1]

        pendulum.control = control
...
plt.grid()
plt.title("Responces, PID control")
plt.xlabel('time, s')
